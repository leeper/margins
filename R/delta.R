...
function(data, 
    ...
    FUN <- .build_grad_fun(data = data, model = model, type = type, weights = weights, eps = eps, ...)
    jacobian <- jacobian(FUN, coef(model), eps = eps)
    vout <- diag(jacobian %*% vcov %*% t(jacobian))
    return(vout)
}

.build_grad_fun <- function(data, model, type = "response", weights = NULL, eps = 1e-7, ...) {
    
    # factory function to return prediction holding data constant but varying coefficients
    FUN <- function(coefs) {
        model[["coefficients"]] <- coefs
        if (is.null(weights)) {
            means <- colMeans(marginal_effects(model = model, data = data, type = type, eps = eps, ...), na.rm = TRUE)
        } else {
            me_tmp <- marginal_effects(model = model, data = data, type = type, eps = eps, ...)
