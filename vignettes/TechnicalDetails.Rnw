...
<<cplot1, echo=FALSE, results = "show", fig.cap = "Marginal Effect of GDP per Capita on Population Size, as shown by Expected Value Plot", warning=FALSE>>=
 library(ggplot2)
 xvals <- seq(7, 10, length.out = 100)
 dat <- cplot(m1, x = 'loggdp', n = 50, xvals = xvals, draw = FALSE)
 lab <- data.frame(x = 7,
                   y = 17)
 seg <- data.frame(x1 = 8, y1 = dat$yvals[dat$xvals==8],
                   x2 = 9, y2 = dat$yvals[dat$xvals==9],
                   x3 = 8, y3 = dat$yvals[dat$xvals==9],
                   x4 = 9, y4 = dat$yvals[dat$xvals==9],
                   x5 = 8, y5 = dat$yvals[dat$xvals==8],
                   x6 = 8, y6 = dat$yvals[dat$xvals==9])
 cplot(m1, x = 'loggdp', n = 50) + 
       xlab('GDP Per Capita (Logged)') +
       xlim(c(7, 10)) +
       ylim(c(-30,80)) +
       geom_segment(aes(x = x1, y = y1, xend = x2, yend = y2, colour = "segment"), data = seg) +
       geom_segment(aes(x = x3, y = y3, xend = x4, yend = y4, colour = "segment"), data = seg) +
       geom_segment(aes(x = x5, y = y5, xend = x6, yend = y6, colour = "segment"), data = seg) +
       annotate(geom="text", x=7.5, y=17, label=expression(Delta(y) == -26.4), color="red", size = 8) +
       annotate(geom="text", x=8.5, y=0, label=expression(Delta(x) == 1), color="red", size = 8) +
       annotate(geom="text", x=9, y=25, label=expression(ME(x) == -26.4), color="red", size = 8) +
