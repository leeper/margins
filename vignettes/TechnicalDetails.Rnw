...
\section{Package Functionality}\label{ref:behavior}
...
\multirow{4}{*}{Core} & \texttt{dydx()} & Calculate marginal effect of one, named variable \\ \cline{2-3}
 ...
 & \texttt{margins()} & An S3 generic to calculate marginal effects for all variables and their variances \\ \cline{2-3}
 & \texttt{prediction()} & An S3 generic that provides a type-safe wrapper around \texttt{predict()} in order to always return a data frame \\ 
\midrule
...
\multirow{4}{*}{Utilities} & \texttt{build\_margins()} & The workhorse function underlying \texttt{margins()} that assembles the response \texttt{"margins"} object for one data frame input. \\ \cline{2-3}
 & \texttt{extract\_marginal\_effects()} & A utility to return a tidy data frame containing only marginal effects from a \texttt{"margins"} object. \\ \cline{2-3}
 & \texttt{build\_datalist()} & A function that takes a data frame and list of replacement values as input and returns a list of data frames containing modified values \\ \cline{2-3}
 & \texttt{find\_data()} & A function to extract or attempt to find the data for a model object. \\
...
\noindent Additionally, the \textbf{interplot} \citep{SoltHuKenkel2016} and \textbf{plotMElm} \citep{Gandrud2016} packages offer visualization functionality specifically for displaying results of models that include multiplicative interaction terms but do not provide general interfaces for calculating marginal effects from arbitrary models. The latter package only handles linear models. \textbf{interactionTest} \citep{EsareySumner2015} offers a useful \texttt{fdrInteraction()} for calculating a $t$-statistic that limits the false discovery rate for a marginal effect based on an interaction, but only works in the case of two-way interaction terms. Relatedly, \textbf{visreg} \citep{BrehenyBurchett2016} provides a \textbf{lattice}-based visualization function, \texttt{visreg()} to visualize predicted values from models in sometimes complex ways. \textbf{margins} attempts to emulate some of this in the \texttt{cplot()} function.
