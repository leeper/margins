...
As an example, if we wanted to know if the marginal effect of horsepower (`hp`) on fuel economy differed across different types of automobile transmissions, we could simply use `at` to obtain separate marginal effect estimates for our data as if every car observation were a manual versus if every car observation were an automatic.
...
suppressMessages(t(sapply(m, summary))[,-1])
```
...
As you can see above, the result is a a relatively long list of `"margins"` objects. Looking at them directly might be difficult - though we could examine any one particular combination of covariates (the particular combination being reported in the output):
...
```{r, fig.height=4, fig.width=8}
cdat <- cplot(x1, "wt", "drat", what = "effect", draw = FALSE)[[1]]
ggplot(cdat, aes(x = xvals)) + 
  geom_line(aes(y = yvals)) +
  geom_line(aes(y = upper), linetype = 2) +
  geom_line(aes(y = lower), linetype = 2) +
