...
As you can see above, the result is a relatively long data frame (basically a stack of `"margins"` objects specific to each requested intersection of `at` values). We can examine the whole stack, but it's an extremely long output (one row for each estimate times the number of unique combinations of `at` values, so 120 rows in this instance). 
