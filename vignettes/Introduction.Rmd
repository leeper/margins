...
```{r}
cplot(x, "wt", what = "prediction")
cplot(x, "wt", what = "effect")
```
...
An easier way to understand all of this is to use graphing. The `cplot()` function, in particular, is useful here. We can, for example, plot the marginal effect of axle ratio across levels of vehicle weight, separately for automatic vehicles (`am=0`) and manual vehicles (`am=1`):

```{r, fig.height=4, fig.width=8}
cplot(x1, x = 'wt', dx = 'drat', z = 'am', zvals = 0:1, what = 'effect')
```
...
cplot(x1b, x = 'wt', dx = 'wt', z = 'am', zvals = 0:1, what = 'effect')
```
